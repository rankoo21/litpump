 ...
 *
 * Balances come from the server `/api/holders` endpoint, which reads
 * `balanceOf` straight from the token contract. That's the single source of
 * truth — every browser sees identical numbers, unlike reconstructing from a
 * (windowed, paginated) trade list. The endpoint polls fast and is hit on
 * every trade event, so it stays close to live.
 */
